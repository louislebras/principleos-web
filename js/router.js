...
//       if (entry.pages.includes(normPath)) {
//         return entry.group;
//       }
//     }
//     return "default";
...
//     const pathToUrl = (path) => (path === "/index" ? "/" : path);

//     // Set hrefs with circular forward, but not backward from home
//     if (prevBtn) {
//       if (currentIndex > 0) {
...
//     // Update button inner text from loaded page content
//     if (prevBtn) {
//       const label = doc
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function normalizePath(url) {
    ...
    // Nettoyer les doubles slash, mais préserver le préfixe de langue
    path = path.replace(/\/{2,}/g, "/");

    if (path === "" || path === "/") {
      return `/${availableLanguages[0]}/index`; // Utiliser la langue par défaut
    }
    if (path.endsWith("/")) {
      path = path.slice(0, -1);
    }
